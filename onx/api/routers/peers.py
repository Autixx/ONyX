...
from onx.db.models.openvpn_cloak_service import OpenVpnCloakServiceState
from onx.db.models.peer import Peer
from onx.db.models.wg_service import WgServiceState
...
def update_peer_config(
    ...
    if payload.wg_service_id is not None:
        peer.wg_service_id = payload.wg_service_id
    if payload.openvpn_cloak_service_id is not None:
        peer.openvpn_cloak_service_id = payload.openvpn_cloak_service_id
    db.add(peer)
    db.commit()
    db.refresh(peer)
    return peer
...
def revoke_peer(peer_id: str, db: Session = Depends(get_database_session)) -> Response:
    ...
    wg_service_id = peer.wg_service_id
    openvpn_cloak_service_id = peer.openvpn_cloak_service_id
    ...
    if wg_service_id:
        wg_service = wg_service_manager.get_service(db, wg_service_id)
        if wg_service is not None and wg_service.state == WgServiceState.ACTIVE:
            wg_service_manager.apply_service(db, wg_service)
    if xray_service_id:
    ...
    if openvpn_cloak_service_id:
        service = openvpn_cloak_service_manager.get_service(db, openvpn_cloak_service_id)
        if service is not None and service.state == OpenVpnCloakServiceState.ACTIVE:
            openvpn_cloak_service_manager.apply_service(db, service)
    event_log_service.log(
        ...
        details={
            ...
            "xray_service_id": xray_service_id,
            "awg_service_id": awg_service_id,
            "wg_service_id": wg_service_id,
            "openvpn_cloak_service_id": openvpn_cloak_service_id,
        },
    )
