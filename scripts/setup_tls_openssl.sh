...
UPSTREAM_PORT="${UPSTREAM_PORT:-10086}"
HTTPS_PORT="${HTTPS_PORT:-443}"
TLS_DOMAIN="${TLS_DOMAIN:-}"
TLS_IP="${TLS_IP:-}"
TLS_CERT_DAYS="${TLS_CERT_DAYS:-825}"
TLS_CERT_DIR="${TLS_CERT_DIR:-/etc/wgdashboard/tls}"
NGINX_SITE_NAME="${NGINX_SITE_NAME:-wg-dashboard-tls}"
FORCE_REGEN="${FORCE_REGEN:-false}"

usage() {
  cat <<'EOF'
Usage: sudo bash scripts/setup_tls_openssl.sh [options]

Options:
  --service-name <name>   systemd service name (default: wg-dashboard)
  --upstream-host <host>  local upstream host for panel proxy (default: 127.0.0.1)
  --upstream-port <port>  local upstream port for panel proxy (default: 10086)
  ...
  --cert-dir <path>       certificate output directory (default: /etc/wgdashboard/tls)
  --nginx-site <name>     nginx site filename prefix (default: wg-dashboard-tls)
